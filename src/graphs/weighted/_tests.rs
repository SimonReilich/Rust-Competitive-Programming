...
pub fn test_new_graph_is_empty (graph: impl Weighted<u32>) {
    assert_eq!(graph.vertex_count(), 0);
    assert_eq!(graph.iter_edges().count(), 0);
}

pub fn test_new_n_initializes_vertices (graph: impl Weighted<u32>, n: usize) {
...
pub mod directed {
    use crate::graphs::{Directed, weighted::Weighted};
    ...
    pub fn test_successors (mut graph: impl Weighted<u32> + Directed) {
        // 0 points to 1 and 2
        graph.add_edge(0, 1, 1);
    ...
    pub fn test_predecessors (mut graph: impl Weighted<u32> + Directed) {
        // 0 -> 2
        // 1 -> 2
        // 2 -> 3
        graph.add_edge(0, 1, 2);
    ...
    pub fn test_self_loop (mut graph: impl Weighted<u32> + Directed) {
    ...
    pub fn test_iter_edges_global (mut graph: impl Weighted<u32>) {
        ...
        let expected = vec![(0, 1, 1), (1, 2, 2), (2, 3, 0)];
...
pub mod undirected {
    ...
    pub fn test_iter_edges_global (mut graph: impl Weighted<u32>) {
        ...
        let expected = vec![(0, 1, 1), (0, 3, 2), (1, 2, 2)];
